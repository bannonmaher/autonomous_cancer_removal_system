...
# through various means of image recognition,
...
# create variables holding the pixel minimum percent match for extraction, and the minimum image and shape percent match

pixel_minimum_percent_match = .05
match_threshold = .85

# initialize variables to hold the current color, shape, or image matching against, the current degree rotation, current degree flexion,  and whether or not a match has been found in the current position
...
# create a function to update the relay board, first establishing a connection to the relay board, if it has not been initialized, or has been dropped, then creating a string sequence of relay states, and sending the states to the relay board
...
# create a function that when called moves the embodiment in by providing power to the corresponding relay attached step motor in the sequence that will move it as desired by one full step sequence
...
# create a function that when called moves the embodiment out by providing power to the corresponding relay attached step motor in the sequence that will move it as desired by one full step sequence
...
# create a function that when called rotates the embodiment forward by providing power to the corresponding relay attached step motor in the sequence that will move it as desired by one full step sequence
...
# create a function that when called rotates the embodiment backward by providing power to the corresponding relay attached step motor in the sequence that will move it as desired by one full step sequence
...
# create a function that when called moves the embodiment arm forward by the degrees in the function parameter by providing power to the corresponding relay attached step motor in the sequence that will move it as desired by full step sequences until the desired degrees have been achieved
...
# create a function that when called moves the embodiment arm backward by the degrees in the function parameter by providing power to the corresponding relay attached step motor in the sequence that will move it as desired by one full step sequence
...
# create a function that when called moves the embodiment head forward by the degrees specified in the parameter by providing power to the corresponding relay attached step motor in the sequence that will move it as desired by full step sequences until the desired angle is achieved
...
# create a function that when called moves the embodiment head backward  by the degrees specified in the parameter by providing power to the corresponding relay attached step motor in the sequence that will move it as desired by one full step sequence
...
# create a function that when called turns the suction machine on then operates the motor connected to the blade wire to move the blade back and forth to cut the tissue, then turns the suction machine off
...
def snapshot():
    ...
    # write the current camera image to a file so that its accessible to the both the system and the user interface
...
def process():
    ...
    for setting in settings:
        ...
        # create a variable for whether or not to remove the tissue if there's a match and set it to the value from the setting
        ...
        # if the recognition mode is color, send the image to the function that checks if the color in the setting exists in the image

        if recognition == "color":
            match_found = image_processing.image_contains_color(camera_image, color, match_threshold)

        # if the recognition mode is shape, send the image to the function that checks if the shape in the setting exists in the image
        ...
        # if a match was found, save the image to an incrementally numbered file and record the time, file name, and setting, to the log file
        ...
        if match_found:
            ...
            # if the tissue match is to be removed according to the setting, call the function to suction cut the tissue
...
# create a function that allows the embodiment to extract tissue in an automated manner through image recognition, where if moving horizontally on x, and a point before that horizontally is clear x with the same y and z, skip=true to next point, if moving vertically and point before vertically, skip to next point, also for rotating, point array is of .2 mm increments as determined by stepper motors

def autonomous():

    # create variable that holds whether or not a match was found by retrieving the result of the function to process tissue at the current location

    match_found = process()

    # while a match was found move in, while incrementing the y position of the embodiment, and then setting whether or not a current match was found by calling the function to process tissue at the current location
    ...
    while match_found:
        ...
        # continue while the current arm rotation degree is less than one revolution, then return the embodiment to zero degrees of rotation
        ...
        while rotation_current < 360:
            ...
            while tissue_current_y >= 0:
                ...
                # proceed while the current arm flexion degree is less than one revolution, then return the embodiment to zero degrees of rotation
...
def main():
    ...
    # retrieve any command line arguments, including  the settings file name, the log file name, and any requested embodiment action
    ...
    # if the setting file name was named snapshot, then call the function that saves a picture of  the current embodiment camera image
    ...
    # proceed if the a snapshot was not specified
        ...
        # if the request to the embodiment was to perform a manually action, send that action to the function that performs manual actions

        if settings_file_name == "manual":
            if action:
                manual(action)

        # if the embodiment is be operated autonomously, retrieve the settings file, and call the function to begin autonomous operation
